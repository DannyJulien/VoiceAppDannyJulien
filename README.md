...
- Explicit to-do lists and grouped item lists in a voice capture or typed note become one titled, individually checkable checklist. List items are never inferred or split into separate actions.
- Checklist owners can add, rename, remove, and reorder items later. A clear capture such as “add lights to my cycling holiday list” can append to that existing open checklist; uncertain matches remain an Inbox proposal instead of changing the wrong list.
...
2. Apply the migrations in `supabase/migrations/` in order. The Supabase CLI is a dev dependency: once per machine run `npx supabase login` (in a real terminal, it opens the browser) and `npx supabase link --project-ref <ref>`, then `npx supabase db push --linked` from `main` applies whatever is missing and `npx supabase migration list --linked` shows local vs. remote state. Do not assume a local migration is already live. In particular, `20260831200000_project_brief_exports.sql` is required before users can copy project briefs, `20260831210000_action_checklist_items.sql` is required before checklist captures can be saved, and `20260901090000_checklist_editing_and_capture_appends.sql` is required before checklist editing and capture appends can be used.
